...
/**
 * Importance of an error in context of pasting. Bigger value means more important error.
 *
 * Various command errors may be encountered when copy/pasting. The importance decides which one to
 * show to the user - it will be one of most important errors, the one that was encountered first.
 */
 ...
 * @see PasteCmdHelper::DoCommand
 */
Money PasteCmdHelper::GetAvailableMoney() const
{
 ...
 *
 * Costs and possible errors will be aggregated.
 *
...
void PasteCmdHelper::DoCommand(TileIndex tile, uint32 p1, uint32 p2, uint32 cmd)
{
	...
	if (_game_mode == GM_EDITOR) {
		ret = ::DoCommand(tile, p1, p2, flags, cmd);
	} else {
		/* test the command */
		ret = ::DoCommand(tile, p1, p2, flags & ~DC_EXEC, cmd);
 ...
 *
 * When collecting a success, the cost must be of type EXPENSES_CONSTRUCTION. Success also
 * indicates that we did something and the "there is nothing to do" error no more applies.
 *
...
void PasteCmdHelper::CollectCost(const CommandCost &cost, TileIndex tile, StringID error_summary)
{
	if (cost.Succeeded()) {
		...
		assert(_game_mode == GM_EDITOR || cost.GetCost() == 0 || cost.GetExpensesType() == EXPENSES_CONSTRUCTION);
 ...
 *
 * The function works similary to PasteCmdHelper::CollectCost. It's specialized
 * in collecting paste errors.
 *
 * @param tile The tile the error concerns.
...
static uint CalcMaxPasteRange(uint curr_h1, uint new_h1, uint curr_h2, uint new_h2, uint length)
{
	...
	uint max_curr_h = min((curr_h1 + curr_h2 + length) / 2, _settings_game.construction.max_heightlevel);
	uint min_new_h = CeilDiv(max<int>(new_h1 + new_h2 - length, 0), 2);
	uint max_new_h = min((new_h1 + new_h2 + length) / 2, _settings_game.construction.max_heightlevel);
...
static bool CopyPasteAreasMayColide(const CopyPasteParams &copy_paste)
{
	...
	/* Calculate the transformation from destination corners to source corners. */
	TileTransformation to_src_corner = InvertTileTransformation(copy_paste.CornerTransform());

	/* calculate current and new heights on destination area corners */
	/* N */
	TileIndex corner = AsMainMapTile(copy_paste.dst_area.tile);
	uint curr_n = TileHeight(corner);
	uint new_n = TileHeight(TransformTile(corner, to_src_corner)) + copy_paste.height_delta;
	/* W */
	corner = AsMainMapTile(copy_paste.dst_area.tile) + TileDiffXY(copy_paste.dst_area.w, 0);
	uint curr_w = TileHeight(corner);
	uint new_w = TileHeight(TransformTile(corner, to_src_corner)) + copy_paste.height_delta;
	/* S */
	corner = AsMainMapTile(copy_paste.dst_area.tile) + TileDiffXY(copy_paste.dst_area.w, copy_paste.dst_area.h);
	uint curr_s = TileHeight(corner);
	uint new_s = TileHeight(TransformTile(corner, to_src_corner)) + copy_paste.height_delta;
	/* E */
	corner = AsMainMapTile(copy_paste.dst_area.tile) + TileDiffXY(0, copy_paste.dst_area.h);
	uint curr_e = TileHeight(corner);
	uint new_e = TileHeight(TransformTile(corner, to_src_corner)) + copy_paste.height_delta;

	/* calculate how far tiles can be altered beyon the paste area (safe approximation) */
	uint range_ne = CalcMaxPasteRange(curr_n, new_n, curr_e, new_e, copy_paste.dst_area.h - 1);
	uint range_sw = CalcMaxPasteRange(curr_s, new_s, curr_w, new_w, copy_paste.dst_area.h - 1);
	uint range_nw = CalcMaxPasteRange(curr_n, new_n, curr_w, new_w, copy_paste.dst_area.w - 1);
	uint range_se = CalcMaxPasteRange(curr_s, new_s, curr_e, new_e, copy_paste.dst_area.w - 1);

	/* calculate the exact area of tile corners which may be altered by the paste process */
	uint x = TileX(AsMainMapTile(copy_paste.dst_area.tile));
	uint y = TileY(AsMainMapTile(copy_paste.dst_area.tile));
	...
	TileArea forbidden_area(
			...
			copy_paste.dst_area.w + range_ne + range_sw,
			copy_paste.dst_area.h + range_nw + range_se);

	/* Include also tile corners at SW and SE borders. */
...
{
	return TileHeight(dst_area.tile) - TileHeight(src_area.tile) + additional_height;
}

enum CopyPastePassType {
	CPPT_TROPIC_ZONE,
	CPPT_NORMAL,
	CPPT_NO_LANDSCAPE,
	CPPT_LANDSCAPE_ONLY,
...
static const CopyPastePassType _copy_paste_normal_pass[] = { CPPT_NORMAL, CPPT_END };
static const CopyPastePassType _copy_paste_editor_pass[] = { CPPT_TROPIC_ZONE, CPPT_NORMAL, CPPT_END };
static const CopyPastePassType _copy_paste_editor_pass_delayed_landscape[] = { CPPT_TROPIC_ZONE, CPPT_NO_LANDSCAPE, CPPT_LANDSCAPE_ONLY, CPPT_END };
...
static inline void DoCopyPaste(const CopyPasteParams &copy_paste)
{
	...
	if ((copy_paste.mode & CPM_TERRAFORM_MASK) == CPM_TERRAFORM_FULL) {
		CopyPasteHeights(copy_paste.src_area, copy_paste);
	}

	const CopyPastePassType *pass;
	if (_game_mode != GM_EDITOR) {
		pass = _copy_paste_normal_pass;
	} else if (IsMainMapTile(copy_paste.dst_area.tile) && (
			/* When pasting ground and trees, it must be done AFTER terraforming
			* (because terraforming clears tiles). If we are in CPM_TERRAFORM_NONE
			* or CPM_TERRAFORM_FULL mode, the terraforming is already done (see
			* above). In CPM_TERRAFORM_MINIMAL mode, tiles will be terraformed while
			* pasting thus we need to split the pasting into two stages - firstly
			* with terrafroming but without landscape, secondly landscape only. */
			((copy_paste.mode & (CPM_WITH_GROUND | CPM_WITH_TREES)) && ((copy_paste.mode & CPM_TERRAFORM_MASK) == CPM_TERRAFORM_MINIMAL)) ||
			/* Split the stages also when pasting both the ground type and
			 * industries so we can assign proper industry IDs to farm fields. */
			((copy_paste.mode & (CPM_WITH_GROUND | CPM_WITH_INDUSTRIES)) == (CPM_WITH_GROUND | CPM_WITH_INDUSTRIES)
			&& (IsMainMapTile(copy_paste.src_area.tile) || ClipboardIndustry::HasItems(MapOf(copy_paste.src_area.tile)))))) {
		pass = _copy_paste_editor_pass_delayed_landscape;
	} else {
		pass = _copy_paste_editor_pass;
	}

	for (; *pass != CPPT_END; pass++) {
		/* Skip CPPT_TROPIC_ZONE if there is no need for it. */
		if (*pass == CPPT_TROPIC_ZONE && (!(copy_paste.mode & CPM_WITH_GROUND) || _settings_game.game_creation.landscape != LT_TROPIC)) continue;

		CopyPasteParams params = copy_paste;
		if (*pass == CPPT_NO_LANDSCAPE) params.mode &= ~(CPM_WITH_GROUND | CPM_WITH_TREES);
		if (*pass == CPPT_LANDSCAPE_ONLY) params.mode &= (CPM_WITH_GROUND | CPM_WITH_TREES);

		for (TransformationTileIteratorT<true> ti(params.src_area, params.TileTransform(), MapOf(params.dst_area.tile)); IsValidTileIndex(ti); ++ti) {
			/* Tropic zone must be set before we try to paste anything e.g. before
			 * we paste a town houses which may require a certain tropic zone. */
			if (*pass == CPPT_TROPIC_ZONE) {
				CopyPastePlaceTropicZone(ti.DstTile(), GetTropicZone(ti.SrcTile()));
			} else {
				CopyPasteTileProc *proc = _tile_type_procs[GetTileType(ti.SrcTile())]->copy_paste_tile_proc;
				if (proc != NULL) proc(ti.SrcTile(), ti.DstTile(), params);
			}
		}
	}
	...
	if (IsMainMapTile(copy_paste.dst_area.tile)) {
		...
		AfterPastingIndustries();
	} else {
		AfterCopyingStations(copy_paste);
		AfterCopyingObjects(copy_paste);
		AfterCopyingIndustries(copy_paste);
	}
}
...
{
	if (!IsValidTile(ta.tile) || ta.w <= 0 || ta.h <= 0) return CMD_ERROR;

	if (_networking) {
		if (ta.w > _settings_game.construction.clipboard_capacity ||
				ta.h > _settings_game.construction.clipboard_capacity) {
			return CMD_ERROR;
		}
	}
...
{
	if (mode & (_game_mode == GM_EDITOR ? ~CPM_MASK_SCENEDIT : ~CPM_MASK)) return false;
...
static void CopyToClipboard(Map *buffer, const TileArea &ta)
{
	...
	CopyPasteParams copy_paste = {
		...
		_game_mode == GM_EDITOR ? CPM_COPY_SCENEDIT : CPM_COPY, // mode
...
{
	/* If everything is "already built" then don't point any specific reason. */
	if (_current_pasting->err_message == STR_ERROR_ALREADY_BUILT) {
		_current_pasting->err_summary = STR_ERROR_CAN_T_PASTE_HERE;
		_current_pasting->err_tile = INVALID_TILE;
	}
	...
	if (_current_pasting->had_success) {
		...
		const PasteErrorImportance min_importance = GetPasteErrorImportance(INVALID_STRING_ID);
		if ((_current_pasting->dc_flags & DC_EXEC) && IsLocalCompany() &&
				GetPasteErrorImportance(_current_pasting->err_message) >= min_importance) {
...
static CommandCost PasteFromClipboard(Map *buffer, TileIndex tile, DoCommandFlag flags, CopyPasteMode mode, DirTransformation transformation, RailType railtype, int additional_height_delta)
{
	...
	copy_paste.height_delta = CalcCopyPasteHeightDelta(copy_paste.src_area, copy_paste.dst_area, additional_height_delta);
 ...
 *    \li bits  0..13 [14] - width of area to copy
 *    \li bits 14..27 [14] - height of area to copy
...
CommandCost CmdCopyToClipboard(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	TileArea src_area(tile, GB(p2, 0, 14), GB(p2, 14, 14));
 ...
 *    \li bits 12..16  [5] - additional amount of tile heights to add to each tile (-16..15)
 *    \li bits 17..19  [3] - transformation to perform (DirTransformation)
 *    \li bits 20..29 [10] - mode (CopyPasteMode)
 *    \li bits 30..31  [2] - [ unused ]
 * @param text Unused.
 * @return The cost of this operation or an error.
 */
CommandCost CmdPasteFromClipboard(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	CopyPasteMode mode = (CopyPasteMode)GB(p2, 20, 10);
	if (!ValParamCopyPasteMode(mode)) return CMD_ERROR;

	/* extract and validate rail type */
	RailType railtype = (RailType)GB(p1, 28,  4);
	if (_game_mode != GM_EDITOR && !ValParamRailtype(railtype)) return CMD_ERROR;

	/* extract transformation and additional height delta */
	DirTransformation transformation = (DirTransformation)GB(p2, 17, 3);
	int additional_height_delta = GB(p2, 12, 5);
	additional_height_delta |= -(additional_height_delta & (1 << 4)); // propagate sign bit
...
CommandCost CmdInstantCopyPaste(TileIndex tile, DoCommandFlag flags, uint32 p1, uint32 p2, const char *text)
{
	...
	copy_paste.transformation = (DirTransformation)GB(p2, 17, 3);
	...
	/* extract and validate copy/paste mode */
	copy_paste.mode = (CopyPasteMode)GB(p2, 20, 10);
	if (!ValParamCopyPasteMode(copy_paste.mode)) return CMD_ERROR;

	/* extract and validate rail type */
	copy_paste.railtype = (RailType)GB(p1, 28,  4);
	if (_game_mode != GM_EDITOR && !ValParamRailtype(copy_paste.railtype)) return CMD_ERROR;

	/* extract the additional number of height units */
	int additional_height_delta = GB(p2, 12, 5); // this is a 5-bit SIGNED integer (-16..15)
	additional_height_delta |= -(additional_height_delta & (1 << 4)); // propagate the sign bit
...
/**
 * Check tile ownership in context of copy-pasting.
 *
 * It is allowed to copy anything from the clipboard (clipbard content is ownerless).
 * When copying from the main map, the content must be owned by the company that is copying.
 *
 * @param tile the tile to test
 * @param company the company to test against, #INVALID_COMPANY to test against current company
 * @return \c true iff the tile is not a main map tile or the tile is owned by the given \c company
 */
bool CopyPasteCheckOwnership(GenericTileIndex tile, CompanyID company)
{
	if (!IsMainMapTile(tile)) return true;

	if (_game_mode == GM_EDITOR) {
		return GetTileOwner(tile) >= MAX_COMPANIES;
	}

	if (company == INVALID_COMPANY) company = _current_company;
	return IsTileOwner(AsMainMapTile(tile), company);
}
